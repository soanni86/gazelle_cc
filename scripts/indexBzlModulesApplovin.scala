...
/**
 * Clones or updates the applovin-bazel-registry repository
 * @return
 *   path to the checked out repository
 */
def checkoutModulesRegistry()(using config: Config): os.Path = {
  val repoDir = config.cacheDir / "applovin-bazel-registry"
  ...
  if os.exists(repoDir) then
    ...
    os.proc(s"git clone git@github.com:AppLovin/applovin-bazel-registry.git --depth=1 $repoDir"
          .split(" "))
